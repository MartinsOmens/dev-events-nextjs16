...
const isNonEmptyString = (value: string): boolean => typeof value === 'string' && value.trim().length > 0;
...
const eventSchema = new Schema<Event, Model<Event>>(
  {
    ...
    agenda: {
      ...
      required: true,
      validate: {
        validator: (value: string[]): boolean => Array.isArray(value) && value.length > 0 && value.every(isNonEmptyString),
...
eventSchema.pre<EventDocument>('save', function preSave(next) {
  ...
  for (const field of requiredStrings) {
    ...
    if (typeof value !== 'string' || value.trim().length === 0) {
      return next(new Error(`Field "${String(field)}" is required and must not be empty`));
    }
  }
  ...
  if (!Array.isArray(this.tags) || this.tags.length === 0) {
    return next(new Error('Tags are required and must contain at least one tag'));
  }
